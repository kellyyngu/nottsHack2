...
class IdentityKeyManager {
   ...
   */
// Replace the existing getKeysInCreation in your IdentityKeyManager class
getKeysInCreation() {
  return KEY_SPECS.map((spec) => {
    ...
    // Ensure we use a clean Uint8Array. Buffer.from().buffer gives the underlying 
    // ArrayBuffer, which is what the WASM layer usually prefers.
    const buf = Buffer.from(key.publicKey, 'hex');
    const pubKeyData = new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength);
